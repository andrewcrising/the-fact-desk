  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[1fr_420px]">
      <section className="panel rounded-3xl p-6">
        <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          <div>
            ...
            <p className="mt-4 max-w-3xl text-sm leading-7 text-slate-400">
              The intake view shows an AI execution event entering RIDGE as evidence material. RIDGE records and canonicalizes execution semantics; it does not alter, block, schedule, or route the execution.
            </p>
          </div>
          <StatusBadge status={artifactDigest ? "canonicalized" : "pending"} label={artifactDigest ? "Canonical artifact generated" : "Awaiting generation"} />
        </div>
        ...
        >
          {isGenerating ? "Generating..." : "Generate Canonical Artifact"}
        </button>

        {artifactDigest ? (
          <div className="mt-6 rounded-2xl border border-ridge-mint/30 bg-ridge-mint/10 p-4">
            <p className="text-sm font-semibold text-ridge-mint">Local canonical artifact digest generated</p>
            <p className="mt-2 font-mono text-sm text-slate-200">{artifactDigest}</p>
          </div>
        ) : null}
      </section>
