...
export function CertificationRecord() {
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[1fr_440px]">
      <section className="space-y-6">
        <div className="panel rounded-3xl p-6">
          <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
            <div>
              <p className="font-mono text-xs uppercase tracking-[0.24em] text-ridge-cyan">Sample certification artifact</p>
              <h1 className="mt-3 break-all text-3xl font-semibold text-slate-50">{mockCertificate.certificateId}</h1>
              <p className="mt-4 max-w-3xl text-sm leading-7 text-slate-400">
                This demo certification record packages mock replay evidence, a sample session root, the policy manifest ID, tolerance profile, and placeholder signature metadata for external verifier review.
              </p>
            </div>
            <StatusBadge status="verified" label="Sample certification artifact" />
          </div>
        </div>

        <div className="grid gap-4 md:grid-cols-2">
          <Detail label="Sample session root" value={mockCertificate.sessionRoot} />
          <Detail label="Policy manifest ID" value={mockCertificate.policyManifestId} />
          <Detail label="Mock verification digest" value={mockCertificate.verificationDigest} />
          ...
          <Detail label="Placeholder signature metadata" value={`${mockCertificate.signatureMetadata.algorithm} / ${mockCertificate.signatureMetadata.status}`} />
        </div>
      ...
      <CodePanel title="Current mock certificate" subtitle="sample artifact data; client-side downloadable JSON" value={mockCertificate} />
    </div>
  );
}
